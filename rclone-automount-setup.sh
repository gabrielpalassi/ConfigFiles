...
case "$ACTION" in
  "restart")
    ...
    ;;
  "reconnect")
    cd ~/.config/systemd/user/ && bun i && bun run start
    ;;
esac
EOF
chmod +x ~/.config/systemd/user/rclone-icloud-failure-notify.sh

# Create TypeScript authenticator files
echo "[INFO] Creating rclone iCloud Drive authenticator files..."
cat > ~/.config/systemd/user/index.ts <<'EOF'
...
//
// Get login config and token via browser
//
...
try {
  ...
  const browser = await puppeteer.launch({
    headless: false,
    defaultViewport: null, // Scale viewport to the browser window size
  ...
  // Intercept the requests to /accountLogin to always remember me (set extended_login=true)
  ...
  if (!signInButton) {
    throw new Error("[Error] Could not find sign in button.");
  }
  ...
  console.error("[Error] Failed to get login tokens:", error);
  process.exit(1);
}
...
const remoteName = "iCloudDrive";
console.log(`[INFO] Updating rclone config...`);
const command = `rclone config update ${remoteName} cookies='${config.cookies}' trust_token='${config.token}'`;

try {
  execSync(command, { stdio: "inherit" });
  console.log("[INFO] Successfully authenticated! Restarting service...");
  execSync("systemctl --user restart rclone-icloud.service", { stdio: "inherit" });
} catch (error) {
  console.error("[Error] Failed to update rclone config:", error);
  process.exit(1);
}
EOF

cat > ~/.config/systemd/user/package.json <<'EOF'
{
  "name": "rclone-icloud-authenticator",
  "description": "",
  "version": "0.0.1",
  "type": "module",
  "main": "index.ts",
...
cat > ~/.config/systemd/user/bun.lock <<'EOF'
{
  "lockfileVersion": 1,
  "configVersion": 0,
  "workspaces": {
  ...
  "packages": {
    ...
    "puppeteer": ["puppeteer@24.34.0", "", { "dependencies": { "@puppeteer/browsers": "2.11.0", "chromium-bidi": "12.0.1", "cosmiconfig": "^9.0.0", "devtools-protocol": "0.0.1534754", "puppeteer-core": "24.34.0", "typed-query-selector": "^2.12.0" }, "bin": "lib/cjs/puppeteer/node/cli.js" }, "sha512-Sdpl/zsYOsagZ4ICoZJPGZw8d9gZmK5DcxVal11dXi/1/t2eIXHjCf5NfmhDg5XnG9Nye+yo/LqMzIxie2rHTw=="],
